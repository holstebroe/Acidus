...
> **Implemented, then corrected (2026-09):** `Oscillator::processNextSample` applies a single, shared coupling-network HPF to whichever waveform is currently selected, replacing an earlier square-only, fixed-150 Hz version. A first attempt made the corner track pitch around 80-115 Hz -- this caused a large, audible regression, since that corner sat at or above the fundamental of ordinary TB-303 bass notes, attenuating the played note itself rather than just coloring it. Reverted to a **fixed** corner (`couplingHz_`, default 44.5 Hz, plausible range 30-60 Hz, exposed as the CLAP parameter "Osc Coupling Freq"), cross-checked against RobinSchmidt/Open303's equivalent stage (`highpass1.setCutoff(44.486)`), which is also fixed, not pitch-tracking. See `TB303_RESEARCH_COMPENDIUM.md` Section 5 and `TB303_PARAMETER_CONFIDENCE.md`.
...
>
> **Implemented, then reverted (2026-09):** `Filter::processFaithfulSample` briefly modeled this as a fixed ~9 Hz coupling-pole network inside the resonance feedback loop, replacing this project's original 150-250 Hz version. That caused a large, audible regression (it let nearly the entire ladder output back into the feedback loop, over-driving the ladder's own tanh saturation) and was reverted back to a single one-pole HPF anchored at 150 Hz (plausible range 100-250 Hz) -- cross-checked against RobinSchmidt/Open303's equivalent stage (`TeeBeeFilter::setFeedbackHighpassCutoff(150.0)`), which uses exactly this fixed 150 Hz value. Treat the ~8-10 Hz secondary-source claim in this section as not corroborated by that cross-check. See `TB303_PARAMETER_CONFIDENCE.md` for the full history and remaining gap (Open303 also uses a further ~7.5 Hz notch and ~14 Hz allpass *outside* the feedback loop, not yet implemented here).
